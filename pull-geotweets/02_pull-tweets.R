...
# Functions for tweet extraction
#' Get local time given a UTC time and a point
#'
#' @param datetimeUTC Vector of IDateTime or POSIXct
#' @param geom Set of points (should be sf or sp)
#'
#' @import velox
#' @import sf
#'
...
#' Pull USA tweets
#'
#' @param dates Vector of IDates from which to pulldata
#' @param poly sf object, should be of polygon(s)
#' @param words Character vector of keywords to match on
#' @param n_max Maximum number of tweets per file
#'
#' @import zoo
#' @import data.table
...
#' Extract tweets from single file
#'
#' @param in_file Filename
#' @param col_types Passed on to read_csv
#' @param col_names Passed on to read_csv
#' @param words Character vector of words to match within tweet (optional)
#' @param poly Polygons to join to, dropping if outside
#' @param n_max Maximums rows
#' @param dates Dates to filter on (useful if pulling from a month file but only wanting a limited set of days)
#'
#' @import sf
#' @import data.table
#' @importFrom readr read_csv cols_only
...
# We add #timesup as well
# Extract pre-2017 and post-2017 separately, then combine. Not required but isolates issues.

# NOTE: Commented out for now. Last run by Patrick Baylis on 2018-10-01. Can only be run on server.
