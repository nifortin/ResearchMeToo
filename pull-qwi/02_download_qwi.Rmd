...
# this one will work with the OLD directory structure (prior to R2018Q3)
source("download_qwi_old.R",echo = TRUE)
...
download_tasks <- function (state,subset=TRUE) {
  if ( sys["user"] == "lv39") {
    dat <- download_qwi_old(state,"sa_f_gc_ns_op_u")
  } else {
...
```
A convenience function to download the version files
```{r define_func2, cache=FALSE}
